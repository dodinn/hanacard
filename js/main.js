...
$('.event_box3 .page_act .btn_stop').click(function(e){
  e.preventDefault();
  //.hasClass('on') ==> 클래스명 on을 가지고 있으면 true
  // 없다면 false
...
$('.event_box3 .page_act .btn_play').click(function(e){
  //내장함수 ==> 함수는 javascript나 jQuret가 가지고 있어서
...
tab.mouseover(function(){
  ...
  let index=target.index() //사용자가 마우스를 올린 버튼 그것의 index 번호를 추출
  // console.log(index)
  style.css({opacity:0}) //1.모두안보이기
  style.eq(index).css({opacity:1}) //2.사용자가 마우스를 올린 버튼의 index 번호에 해당하는 .life_style ul li 요소를 보이게 한다.
})